...
const Home = ({ data, sendCmd, primarykey }) => {
  ...
  const [__room_r1, set__room_r1] = useState();
  const [__room_r2, set__room_r2] = useState();
  ...
  useEffect(() => {
    if (data && d.length !== 0) {
      d = data[primarykey];
      setTest(d);
      if (d.hasOwnProperty("r1")) set__room_r1(d["r1"]);
      if (d.r2) set__room_r2(d.r2);
  ...
  const [defaultToggle, setDefaultToggle] = useState({
    livingroom_bottom: test["r1"]
      ? test["r1"]
      : defaultToggle.livingroom_bottom,
    livingroom_main_check: test[""], //거실메인 버튼 초기값
                ...
                <div className="col_wrap_inner_left margin_right">
                  <label className="toggle">
                    <input
                      ...
                      defaultChecked={__room_r1 === 1 ? true : false}
                    />
                    <span className="dot">
